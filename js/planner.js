...
async function _ncpFetch(){
    try{
        var doc = await db.collection('settings').doc(orgKey('ncpPlanners')).get();
...
async function _ncpWrite(planners){
    try{
        ...
        await db.collection('settings').doc(orgKey('ncpPlanners')).set({ planners: planners, orgId: CURRENT_USER.orgId });
